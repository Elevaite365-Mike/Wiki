...
2. **Set a maximum limit** — for example, `3` rows — to avoid infinite loops.
...
When looping through fields you can use the **Inherit field** option. This automatically applies each action — like a click or type — to the current field in the loop.
